    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        if (list_data.length > 0) {
            ...
            data_app += '<thead>' +
                ...
                '<th>Sisa Uang</th>' +
                '<th>Hapus Pengeluaran</th>' +
                '<th>Lihat Lihat Pengeluaran</th>' +
                '<th>Edit Pengeluaran</th>' +
                '</thead> <tbody>';
            ...
            for (i in list_data) {
                ...
                data_app +=
                    ...
                    '<td>' + list_data[i].pengeluaran + ' </td>' +
                    '<td>' + list_data[i].sisa + ' </td>' +
            ...
            data_app = "Tidak ada Pengeluaran hari ini";
        }
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#epengeluaran").val(list_data[i].pengeluaran);
                $("#esisa").val(list_data[i].sisa);
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#lpengeluaran").val(list_data[i].pengeluaran);
                $("#lsisa").val(list_data[i].sisa);
                list_data.splice(idx_data, 1);
            }
            idx_data++;
        }
...
function simpanData() {
    ...
    if (!liff.isInClient()) {
        ...
        id_data++;
        list_data.push({ 'id_data': id_data, 'nama': nama, 'tanggal': tanggal, 'pengeluaran': pengeluaran, 'sisa': sisa });
